...
ApplicationWindow {
    ...
    width: 360
    height: 640
    title: translations.getTranslation("appTitle")
    ...
    // Функция для получения статьи на текущем языке
    function getArticle(articleType) {
        var lang = translations.currentLanguage;
        switch(lang) {
            case "ru":
                return articleType === "radiation" ? articles.radiationArticleRu : articles.geigerArticleRu;
            case "en":
                return articleType === "radiation" ? articles.radiationArticleEn : articles.geigerArticleEn;
            case "de":
                return articleType === "radiation" ? articles.radiationArticleDe : articles.geigerArticleDe;
            case "fr":
                return articleType === "radiation" ? articles.radiationArticleFr : articles.geigerArticleFr;
            case "uk":
                return articleType === "radiation" ? articles.radiationArticleUk : articles.geigerArticleUk;
            case "zh":
                return articleType === "radiation" ? articles.radiationArticleZh : articles.geigerArticleZh;
            case "ja":
                return articleType === "radiation" ? articles.radiationArticleJa : articles.geigerArticleJa;
            default:
                return articleType === "radiation" ? articles.radiationArticleRu : articles.geigerArticleRu;
        }
    }

    // Drawer для меню статей
    Drawer {
        id: articleDrawer
        ...
        Column {
            ...
            Label {
                width: parent.width
                text: translations.getTranslation("appTitle")
                font.pixelSize: 20
                font.bold: true
            ...
            ItemDelegate {
                width: parent.width
                text: translations.getTranslation("aboutRadiation")
                onClicked: {
                    stackView.push("ArticleView.qml", {
                        title: translations.getTranslation("aboutRadiation"),
                        articleText: getArticle("radiation")
                    })
                    articleDrawer.close()
                }
            }
            ...
            ItemDelegate {
                width: parent.width
                text: translations.getTranslation("aboutGeiger")
                onClicked: {
                    stackView.push("ArticleView.qml", {
                        title: translations.getTranslation("aboutGeiger"),
                        articleText: getArticle("geiger")
                    })
                    articleDrawer.close()
                }
            }
            ...
            Label {
                width: parent.width
                text: translations.getTranslation("language")
                font.pixelSize: 16
                font.bold: true
                padding: 10
            }
            ...
            Repeater {
                ...
                delegate: ItemDelegate {
                    width: parent.width
                    text: modelData.name
                    highlighted: translations.currentLanguage === modelData.code
                    onClicked: {
                        translations.currentLanguage = modelData.code;
                        root.title = translations.getTranslation("appTitle");
                    }
                }
            }
        }
    ...
    // StackView для навигации между экранами
    StackView {
        id: stackView
        anchors.fill: parent
